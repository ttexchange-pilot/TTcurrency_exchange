...
window.addEventListener('DOMContentLoaded', ()=>{
  ...
  function applyHeader(company, branch){
    const H=(window.TTConfig.companies?.[company]?.branches?.[branch])||{
      company,
      address:'',
      contact:'',
      Licence:'',
    ...
    const licEl = $('#r-company-licence'); if(licEl) licEl.textContent = H.Licence || '';
    const taxEl = $('#r-company-taxid'); if(taxEl) taxEl.textContent = H.TaxId || '';
  }
  ...
  async function makeReceipt(){
    ...
    // ตั้งหัวข้อเลขบัตรในใบเสร็จให้ตรงกับที่เลือก
    const rCidLabel = $('#r-cid-label');
    if(rCidLabel && cidTypeSel){
      const t = cidTypeSel.value;
      if(t==='id') rCidLabel.textContent = 'บัตรประชาชน';
      else if(t==='passport') rCidLabel.textContent = 'หนังสือเดินทาง';
      else if(t==='other') rCidLabel.textContent = 'บัตรอื่นๆ';
      else rCidLabel.textContent = 'Passport/ID';
    }
